...
class InstantDBLogging {
  ...
  static String _instanceId = 'Instance-${DateTime.now().millisecondsSinceEpoch % 10000}';
  ...
  static void _logHandler(LogRecord record) {
    ...
    final formattedMessage = '[$timestamp] [$_instanceId] [$logger] [$level] $message$error$stackTrace';
  ...
  static void logTransaction(String phase, String txId, {
    int? operationCount,
    String? entityType,
    String? status,
    int? duration,
  ...
  static void logWebSocketMessage(String direction, String messageType, {
    String? eventId,
    int? messageSize,
  }) {
    final data = correlationData(
    ...
    logWithCorrelation(webSocket, Level.FINE, 'WS_$direction$messageType', data);
  }
  
  static void logQueryEvent(String event, String queryKey, {
    String? reason,
    int? resultCount,
  }) {
    final data = correlationData(
