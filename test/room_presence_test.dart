  ...
  group('Room-based Presence System Tests', () {
    ...
    group('joinRoom API', () {
      ...
      test('should initialize room data when joining', () {
        ...
        expect(presence.getPresence('room-1'), isA<Signal<Map<String, PresenceData>>>());
        expect(presence.getCursors('room-1'), isA<Signal<Map<String, CursorData>>>());
        expect(presence.getTyping('room-1'), isA<Signal<Map<String, DateTime>>>());
        expect(presence.getReactions('room-1'), isA<Signal<List<ReactionData>>>());
      });

      test('should support initial presence data', () {
        const initialPresence = {'status': 'online', 'mood': 'happy'};
        final room = presence.joinRoom('room-1', initialPresence: initialPresence);
    ...
    group('Topic Pub/Sub System', () {
      test('should handle topic subscriptions per room', () {
        ...
        expect(sub1, isNot(same(sub2))); // Different streams for different rooms
      });
    ...
    group('API Compatibility', () {
      test('should maintain backward compatibility with direct room ID methods', () {
